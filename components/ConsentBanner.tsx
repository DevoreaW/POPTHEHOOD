...
const ConsentBanner: React.FC<ConsentBannerProps> = ({ onAccept }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black/85 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="bg-[#3a3a40] border border-[#4a4a52]/60 rounded-2xl p-8 max-w-md w-full shadow-2xl">
        ...
        <div className="flex items-center gap-4 mb-6">
          ...
          <div>
            ...
            <h2 className="text-lg font-bold text-[#f1f5f9] leading-none" style={body}>
              Before you begin
            </h2>
            <p className="text-xs text-slate-400 mt-1" style={body}>
          ...
          <label className="flex items-start gap-3 cursor-pointer">
            <div className="relative mt-0.5 flex-shrink-0">
              <input
                ...
                className="sr-only"
              />
              <div className={`w-5 h-5 rounded-md border-2 flex items-center justify-center transition-all ${ageConfirmed ? 'bg-orange-500 border-orange-500' : 'border-[#4a4a52] bg-black/40'}`}>
                {ageConfirmed && (
            ...
            <span className="text-slate-400 text-sm leading-snug" style={body}>
              I confirm that I am{' '}
              <strong className="text-[#f1f5f9] font-semibold">13 years of age or older</strong>
            </span>
          </label>
          ...
          <label className="flex items-start gap-3 cursor-pointer">
            <div className="relative mt-0.5 flex-shrink-0">
              <input
                ...
                className="sr-only"
              />
              <div className={`w-5 h-5 rounded-md border-2 flex items-center justify-center transition-all ${termsAccepted ? 'bg-orange-500 border-orange-500' : 'border-[#4a4a52] bg-black/40'}`}>
                {termsAccepted && (
                  <svg className="w-3 h-3 text-[#f1f5f9]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="3" d="M5 13l4 4L19 7" />
                  </svg>
        ...
        <button
          ...
          disabled={!canAccept}
          className={`not-italic w-full py-3 rounded-xl transition-all focus:outline-none focus:ring-2 focus:ring-orange-500 focus:ring-offset-2 focus:ring-offset-[#2a2a2e]
            ${canAccept
              ...
              : 'bg-black/40 border border-[#4a4a52] text-slate-400 cursor-not-allowed'
            }`}
